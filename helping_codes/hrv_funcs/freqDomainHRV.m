...
function output = freqDomainHRV(ibi,VLF,LF,HF,AR_order,window,noverlap,nfft,fs,methods,flagPlot)
...
function plotPSD(aH,F,PSD,VLF,LF,HF,limX,limY)
    ...
    hold(aH);
    area(aH,F(iVLF(1):iVLF(end)+1),PSD(iVLF(1):iVLF(end)+1),'FaceColor',[0.9882 0.902 0.902]);
    area(aH,F(iLF(1):iLF(end)+1),PSD(iLF(1):iLF(end)+1),'FaceColor',[0.9255 0.9255 0.9882]);
    area(aH,F(iHF(1):iHF(end)+1),PSD(iHF(1):iHF(end)+1),'FaceColor',[1 1 0.8863]);
