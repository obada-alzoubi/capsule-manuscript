...
function output = timeFreqHRV(ibi,nibi,VLF,LF,HF,AR_order,winSize,overlap,nfft,fs,methods)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if flagWavelet
        ...
        variance=var(y);
        n=length(y);
        output.wav.hrv=calcWavHRV(f,power,scale,Cdelta,variance,n,dj,dt,VLF,LF,HF);
...
% Ref: Modified from Gary Clifford's ECG Toolbox: calc_lfhf.m   
    ...
    output= struct('aVLF',z, 'aLF',z, 'aHF',z, 'aTotal',z, 'pVLF',z, ...
        'pLF',z, 'pHF',z, 'nLF',z, 'nHF',z, 'LFHF',z, 'rLFHF',0,'peakVLF',z, ...
        'peakLF',z, 'peakHF',z);
...
%   
%
% Ref: This code is based on the calc_lfhf.m function from Gary Clifford's ECG Toolbox.    
