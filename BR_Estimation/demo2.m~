...
for i=1:Nwin
    ...
    ecg_win = 1000*ecg(idx_start(i):idx_stop(i));
    
    [~, R_win] = findpeaks(ecg_win,'MinPeakDistance',0.5,...
    'MinPeakHeight',0.15, 'MinPeakProminence',0.3);
    % estimate the RR
    R_win= R_win';
    resp_asdf(i) = 60./ECG_calcRespRate(ecg_win, fs, [zeros(size(R_win)); R_win; zeros(size(R_win))], ecg_feature, 'auto_corr');
    resp_fft(i) = 60./ECG_calcRespRate(ecg_win, fs, [Q_win; R_win; S_win], ecg_feature, 'fft');
    resp_zc(i) = 60./ECG_calcRespRate(ecg_win, fs, [Q_win; R_win; S_win], ecg_feature, 'R_amplitude');
